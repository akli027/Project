...
{r load dataset}
library(readr)
game_overview <- read_csv("game_overview.csv")
komen <- game_overview$overview
...
myStopwords = readLines("stopword-eng.txt")
...
library(wordcloud2)
library(wordcloud)
reviewsteamclean<- vroom(here("commentclean.csv"))
reviewnew<- reviewsteamclean$text
ui <- fluidPage(
  titlePanel("Analisa"),
  mainPanel(
    ...
    tabsetPanel(type = "tabs",
                ...
                tabPanel("Wordcloud",plotOutput(outputId = "plot_word_cloud", height = "1400px"))
    )
  )
)
...
server <- function(input, output) {
  ...
  output$tbl = DT::renderDataTable({
    DT::datatable(reviewsteamclean, options = list(lengthChange = FALSE))
  })
  ...
  output$Wordcloud <- renderPlot({
    ...
    df<-df[sample(nrow(df)),]
    glimpse(df)
    corpus<-Corpus(VectorSource(reviewnew))
    corpus
    inspect(corpus[1:10])
    #fungsinya untuk membersihkan data data yang tidak dibutuhkan 
    corpus.clean<-corpus%>%
    ...
    dtm.test<-dtm[51:64]
    corpus.clean.train<-corpus.clean[1:50]
    corpus.clean.test<-corpus.clean[51:64]
    dim(dtm.train)
    fivefreq<-findFreqTerms(dtm.train,5)
    length(fivefreq)
    dtm.train.nb<-DocumentTermMatrix(corpus.clean.train,control = list(dictionary=fivefreq))
